...
// builds a profile: website, LinkedIn, team size, specialties, pain points.
...
export async function executeResearchStep(step) {
  ...
  try {
    ...
    // 3. Update ca_brokers with research results
    await supabase
      .from('ca_brokers')
      .update({
    ...
    // 5. Mark step and mission complete
    await supabase
      .from('ops_mission_steps')
      .update({
        status: 'completed',
