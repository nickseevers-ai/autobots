...
async function checkProactiveProspector(sb, conditions, actionConfig) {
  ...
  // Queue all fetched brokers as separate steps in one mission
  const broker = brokers[0]; // use first broker for mission title

  return {
    fired: true,
    proposal: {
      agent_id: actionConfig.target_agent,
      title: `Research CA Brokers: batch of ${brokers.length} (starting ${broker.name})`,
      description: `Research online presence and business profile for ${brokers.length} CA brokers`,
      proposed_steps: brokers.map(b => ({
        kind: 'research',
        payload: {
          broker_id: b.id,
...
async function checkProactiveContentOpportunity(sb, conditions, actionConfig) {
  ...
  return {
    ...
    proposal: {
      agent_id: actionConfig.target_agent,
      title: `Write outreach emails: batch of ${brokers.length} (starting ${broker.name})`,
      description: `Generate personalized cold outreach emails for ${brokers.length} CA brokers`,
      proposed_steps: brokers.map(b => ({
        kind: 'write_content',
        payload: {
          broker_id: b.id,
...
export async function runHeartbeat() {
  ...
  try {
    ...
    // 1. Evaluate triggers
    try {
      summary.triggers = await evaluateTriggers(getSupabase(), 4000);
    } catch (error) {
    ...
    try {
      summary.recovery = await recoverStaleSteps(getSupabase());
    } catch (error) {
    ...
    // Log to audit table
  await getSupabase().from('ops_action_runs').insert({
