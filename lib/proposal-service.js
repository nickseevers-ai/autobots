...
const STEP_KIND_GATES = {
  ...
  scrape_race_data: checkScrapeRaceDataGate,
};
...
async function checkScrapeRaceDataGate(sb) {
  const policy = await getPolicy(sb, 'daily_limits');
  const todayCount = await countTodaySteps(sb, 'scrape_race_data');

  if (todayCount >= (policy.max_scrape_per_day || 50)) {
    return { ok: false, reason: `Daily scrape quota reached (${todayCount}/50)` };
  }
  return { ok: true };
}
