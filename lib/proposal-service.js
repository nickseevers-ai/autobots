...
async function checkResearchGate(sb) {
  ...
  if (todayCount >= (policy.max_research_per_day || 20)) {
    return { ok: false, reason: `Daily research quota reached (${todayCount}/20)` };
  }
  return { ok: true };
}
...
async function checkWriteContentGate(sb) {
  ...
  if (todayCount >= (policy.max_content_per_day || 20)) {
    return { ok: false, reason: `Daily content quota reached (${todayCount}/20)` };
  }
  return { ok: true };
}

async function checkScrapeDREGate(sb) {
  const policy = await getPolicy(sb, 'daily_limits');
  const todayCount = await countTodaySteps(sb, 'scrape_dre');

  if (todayCount >= (policy.max_dre_scrape_per_day || 3)) {
    return { ok: false, reason: `Daily DRE scrape quota reached (${todayCount}/3)` };
  }
  return { ok: true };
}
